...
import SearchCricketLeague from "./SearchCricketLeague";

export default function PopularCricketLeague() {
  const [leagueList, setLeagueList] = useState([]);
  const [isOpen, setIsOpen] = useState(false);
  const { data: cricketLeagues, isLoading: cricketLeaguesLoading, refetch } = useGetPopularLeaguesQuery("cricket");
  const [deleteCricketLeague, { isSuccess: deleteLeagueSuccess, isError: deleteLeagueError }] =
    useDeletePopularLeagueMutation();
  ...
  return (
    <section className={"@5xl:grid @5xl:grid-cols-6"}>
                        ...
                        <Popover.Content>
                          {({ setOpen }) => (
                            <div className='w-56'>
                              ...
                              <Text>Are you sure you want to delete the league?</Text>
                              <div className='flex justify-end gap-3 mb-1'>
                                ...
                                <Button
                                  size='sm'
                                  onClick={() => {
                                    deleteCricketLeague(league._id);
                                    setOpen(false);
                                  }}
                                >
                                  Yes
      ...
      <SearchCricketLeague isOpen={isOpen} setIsOpen={setIsOpen} />
    </section>
  );
}
