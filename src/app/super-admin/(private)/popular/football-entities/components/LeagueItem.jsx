...
function LeagueItem({ league, selectPointTableHandler, deleteLeagueHandler }) {
  const { attributes, listeners, setNodeRef, transform, transition } = useSortable({ id: league.id });

  const style = { transform: CSS.Transform.toString(transform), transition };

  return (
    <div className='mb-2 rounded-md bg-slate-100 p-2' ref={setNodeRef} style={style}>
      <div className='grid grid-cols-8 gap-4 md:grid-cols-12'>
        ...
        <div className='col-span-12 flex items-center justify-center md:col-span-4'>
          <button className='btn btn-error btn-sm  ' onClick={() => deleteLeagueHandler(league)}>
