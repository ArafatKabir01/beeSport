import { mahaScoreBackendUrl } from "@/lib/axios/getAxios";
...
function LeagueDeleteModal({ session, singleLeague, popularLeaguesRefetch }) {
  ...
  const deleteLeagueHandler = async (id) => {
    setSubmitting(true);
    try {
      const { data } = await mahaScoreBackendUrl.delete(`/api/admin/popular-leagues/${id}`, {
    ...
    <dialog id='leagueDeleteModal' className='modal'>
      <div className='modal-box'>
        ...
        <p className='py-4'>
          ...
          <span className='font-medium text-red-500'> {singleLeague?.name} </span>
          League?
        </p>
