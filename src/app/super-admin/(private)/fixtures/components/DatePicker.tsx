...
function DatePicker({ dates, handleDate }: { dates: any; handleDate: any }, flatpickrRef: any) {
  return (
    <div className='grid grid-cols-1 gap-6 pt-5'>
      <div className='panel'>
        <div className='mb-5 flex items-center justify-between'>
          <h5 className='dark: -light text-lg font-semibold'>Pick Your Date</h5>
        </div>
        <div className='mb-5 flex items-center justify-center overflow-x-scroll md:overflow-x-auto'>
          <div className='m-auto flex w-9/12 justify-center border-b-[2px] border-dashed border-[#eaeaea] pb-5'>
            {dates.map((item: any, index: number) => (
              <div onClick={() => handleDate(item)} key={index} className='flex justify-center'>
                <div
                  className={`ml-2 min-w-[5rem] cursor-pointer select-none rounded-full px-4 py-1  ${
                    index == 2 ? "bg-primary font-bold  " : "bg-slate-100 font-medium text-slate-500"
                  }`}
                >
                  <div>
                    <span className='block text-center'>{getIsToday(item) ? "TODAY" : getDay(item)}</span>
