...
export default function LiveMatchCreate({ searchParams }: { searchParams: IFixtureSearchParams }) {
  const router = useRouter();
  const [createLiveMatch, { data: response, isSuccess, isError }] = useCreateLiveMatchMutation();
  ...
  const initialValues = {
    ...
    streaming_sources: getStreamObject(searchParams?.fixture_id ? true : false)
  };
    ...
    const liveMatchData = {
      ...
      team_one_image:
        values.team_one_image_type === ""
          ? `${process.env.NEXT_PUBLIC_MAHASCORE_BACKEND_URL}/public/default/team-logo.png`
  ...
  return (
    <div className='flex-grow pb-10'>
      <div className='grid grid-cols-1 gap-8 @2xl:gap-10 @3xl:gap-12'>
        <Formik initialValues={initialValues} validationSchema={matchSchema} enableReinitialize onSubmit={handleSubmit}>
