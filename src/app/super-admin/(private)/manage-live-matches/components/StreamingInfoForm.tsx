  ...
  return (
    <FormBlockWrapper title='Streaming Sources Information' className='border border-gray-300 rounded-lg p-4'>
      <FieldArray name='streaming_sources'>
        {(arrayHelpers) => (
          <div>
            {values.streaming_sources?.map((streamSource: any, sourceIndex: number) => (
              <div className='mb-4 rounded border border-gray-200 p-4' key={sourceIndex}>
                <div className='flex items-center justify-between'>
                  <p className='text-base font-semibold text-primary'>Streaming Source: {sourceIndex + 1}</p>
                  {sourceIndex > 0 && (
                    <button type='button' onClick={() => arrayHelpers.remove(sourceIndex)}>
                ...
                <div className='grid grid-cols-1 gap-4 md:grid-cols-2'>
                  <Field name={`streaming_sources[${sourceIndex}].stream_title`}>
                  ...
                  <Field name={`streaming_sources[${sourceIndex}].is_premium`}>
                    {({ field }: { field: any }) => (
                      <>
                        <label className='form-control w-full'>
                          <div className='label'>
                            <span className='label-text font-semibold'>Is Premium?</span>
                          </div>
                  ...
                  <Field name={`streaming_sources[${sourceIndex}].resolution`}>
                    {({ field }: { field: any }) => (
                      <>
                        <label className='form-control w-full'>
                          <div className='label'>
                            <span className='label-text font-semibold'>Resolution</span>
                          </div>
                          <select className='select select-bordered' {...field}>
                  ...
                  {/* <Field name={`streaming_sources[${sourceIndex}].portrait_watermark`}>
                    {({ field }: { field: any }) => (
                      <>
                        <label className='form-control w-full'>
                          <div className='label'>
                            <span className='label-text font-semibold'>Portrait Watermark(json)</span>
                          </div>
                          <textarea className='textarea textarea-bordered' {...field}></textarea>
                        </label>
                      </>
                    )}
                  </Field>

                  <Field name={`streaming_sources[${sourceIndex}].landscape_watermark`}>
                    {({ field }: { field: any }) => (
                      <>
                        <label className='form-control w-full'>
                          <div className='label'>
                            <span className='label-text font-semibold'>Landscape Watermark(json)</span>
                          </div>
                          <textarea className='textarea textarea-bordered' {...field}></textarea>
                        </label>
                      </>
                  ...
                  <Field name={`streaming_sources[${sourceIndex}].stream_status`}>
                    {({ field }: { field: any }) => (
                      <>
                        <label className='form-control w-full'>
                          <div className='label'>
                      ...
                      <>
                        <label className='form-control w-full'>
                          <div className='label'>
                            <span className='label-text font-semibold'>
                              Stream Type{" "}
                              <span className='text-red-600'>* {meta.error && <span>({meta.error})</span>}</span>
                            </span>
                          </div>
                          <select className={`select select-bordered ${meta.error && "select-error"}`} {...field}>
                  ...
                  {streamSource.stream_type !== "root_stream" && (
                    <Field name={`streaming_sources[${sourceIndex}].stream_url`}>
                      {({ field, meta }: { field: any; meta: any }) => (
                        <>
                          <label className='form-control w-full'>
                            <div className='label'>
                ...
                {streamSource.stream_type === "restricted" && (
                  <FieldArray name={`streaming_sources[${sourceIndex}].headers`}>
                    {(arrayHelpers) => (
                      <div className='mt-6'>
                        {streamSource.headers.map((headers: any, headersIndex: number) => (
                          <div
                            key={headersIndex}
                            className='relative mt-3 rounded border border-gray-100 bg-gray-50 p-4 shadow'
                          >
                            <div className='grid grid-cols-1 gap-5 md:grid-cols-2'>
                              <Field name={`streaming_sources[${sourceIndex}].headers[${headersIndex}].key`}>
                                {({ field, meta }: { field: any; meta: any }) => (
                                  <>
                                    <label className='form-control w-full'>
                                      <div className='label'>
                              ...
                              <Field name={`streaming_sources[${sourceIndex}].headers[${headersIndex}].value`}>
                                {({ field, meta }: { field: any; meta: any }) => (
                                  <>
                                    <label className='form-control w-full'>
                                      <div className='label'>
                              ...
                              {/* {headersIndex > 0 && ( */}
                              <div className={`${headersIndex > 0 ? "absolute right-1 top-1 block" : " hidden"}`}>
                                <button type='button' onClick={() => arrayHelpers.remove(headersIndex)}>
                                  <RiCloseCircleFill className='rounded text-2xl hover:text-rose-600' />
                                </button>
                              </div>
                              {/* )} */}
                ...
                {/* Root streams */}
                {streamSource.stream_type === "root_stream" && (
                  <FieldArray name={`streaming_sources[${sourceIndex}].root_streams`}>
                    {(arrayHelpers) => (
                      <div className='mt-6'>
                        {streamSource.root_streams?.map((root_stream: any, root_streamIndex: number) => (
                          <div
                          ...
                          >
                            <div className='grid grid-cols-1 gap-5 md:grid-cols-2'>
                              ...
                              >
                                {({ field }: { field: any }) => (
                                  <>
                                    <label className='form-control w-full'>
                                      <div className='label'>
                                        <span className='label-text font-semibold'>Stream Type</span>
                                      </div>
                                      <select className='select select-bordered' {...field}>
                              ...
                              >
                                {({ field }: { field: any }) => (
                                  <>
                                    <label className='form-control w-full'>
                                      <div className='label'>
                                        <span className='label-text font-semibold'>Stream Status</span>
                                      </div>
                                      <select className='select select-bordered' {...field}>
                                        <option value='1'>Active</option>
                                        <option value='0'>Inactive</option>
                              ...
                              >
                                {({ field }: { field: any }) => (
                                  <>
                                    <label className='form-control w-full'>
                                      <div className='label'>
                                        <span className='label-text font-semibold'>Stream Url</span>
                                      </div>
                              ...
                              <div className={`${root_streamIndex > 0 ? "absolute right-2 top-2 block" : " hidden"}`}>
                                <button type='button' onClick={() => arrayHelpers.remove(root_streamIndex)}>
                                  <RiCloseCircleFill className='rounded text-2xl hover:text-rose-600' />
                                </button>
                              </div>
                            </div>
