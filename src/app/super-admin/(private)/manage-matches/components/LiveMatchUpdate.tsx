...
export default function LiveMatchUpdate({ liveMatchId }: { liveMatchId: number }) {
  ...
  const { data: liveMatch, isLoading } = useGetFixtureByIdQuery(liveMatchId);
  ...
  useEffect(() => {
    if (!isLoading) {
      ...
      const updatedData = {
        ...
        team_one_image: matchData?.participants[0]?.image,
        team_two_image: matchData?.participants[1]?.image,
        // team_one_image_type: 'url',
        // team_two_image_type: 'url',
