...
import { FieldClearButton, FieldError, FieldHelperText, cn } from 'rizzui';
  ...
  <div className={cn('rizzui-phone-number', className)}>
    {label && (
      <label className={cn('block', labelClasses.size[size], labelClassName)}>
        {label}{' '}
        {requiredStar && <span className="font-bold text-red-600">*</span>}
      </label>
    )}
    ...
    {!error && helperText && (
      <FieldHelperText size="sm" className={helperClassName}>
        {helperText}
      </FieldHelperText>
    )}
    {error && (
      <FieldError size="sm" error={error} className={errorClassName} />
    )}
  </div>
);
