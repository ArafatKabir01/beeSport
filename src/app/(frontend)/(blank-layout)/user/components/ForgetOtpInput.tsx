import { useResendOtpMutation, useVerifyForgetPasswordOtpMutation } from "@/features/auth/authApi";
...
export default function ForgetOtpInput({
  ...
  const [verifyForgetPasswordOtp, { data: responseData, isSuccess, isError }] = useVerifyForgetPasswordOtpMutation();
    ...
    <div className={`card-body ${currentTab === 1 ? "" : "hidden"}`}>
      ...
      <div className='label mb-4'>
        <span className='text-sm font-semibold  '>
          We sent an OTP to your phone (+{forgetPhone}). You have 2 minutes to complete this verification. Thank you!
        </span>
      </div>
      <form onSubmit={otpSubmitHandler}>
        ...
        {otpValidMsg && <p className='mt-4 px-1 text-center font-medium text-error'>{otpValidMsg}</p>}
        <div className='mt-10 grid grid-cols-12 items-center'>
          <div className='col-span-8'>
            <p className='flex select-none items-center'>
              ...
              {resendOtpSubmitting && <PiSpinnerLight size={20} className='ml-1 animate-spin' />}
            </p>
          </div>
        </div>
        <div className='card-actions mt-5 justify-end'>
          <button type='submit' className='btn btn-primary w-full' disabled={otpSubmitting}>
            Next
            {otpSubmitting && <PiSpinnerLight className='animate-spin text-base' />}
          </button>
        </div>
      </form>
      <div className='mt-5 flex select-none items-center justify-center'>
        <Link href='/' className='flex items-center transition-all duration-150 ease-in hover:text-primary'>
