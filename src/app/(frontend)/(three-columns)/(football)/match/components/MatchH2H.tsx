import MatchScore from "@/app/(frontend)/components/fixtureCardInfo/MatchScore";
...
import { INestedObject, IUserProfile, Team } from "@/types";
...
export default function MatchH2H({ matchData }: { matchData: INestedObject }) {
  ...
  const [addToFavorite] = useAddToUserFavoritesMutation();
  const [updateFavorite] = useUpdateUserFavoritesMutation();
  const { data: userData }: { data?: IUserProfile } = useGetProfileQuery(undefined);

  const homeId = matchData?.data?.participants?.find((team: Team | undefined) => team?.meta?.location === "home")?.id;

  const awayId = matchData?.data?.participants.find((team: Team | undefined) => team?.meta?.location === "away")?.id;

  const { isLoading: headToHeadLoading, data: headToHeadData } = useGetFixturesMatchH2HByTeamQuery(
    { homeId, awayId },
    { skip: !homeId && !awayId }
  );
  ...
  const teamByLocation = (location: string) =>
    matchData?.data?.participants?.find((team: Team | undefined) => team?.meta?.location === location);

  // Handle Add To Favorites
  const handleAddToFavorite = (matchId: number) => {
    setIsFavorite(true);
    addToFavorite({
      phone: userData?.data.phone,
      context: `${sportType}_matches`,
      item: {
        id: matchId
      }
    });
  ...
  // Handle Update Favorites
  const handleUpdateFavorite = (matchId: number) => {
    setIsFavorite(false);
    updateFavorite({
      phone: userData?.data.phone,
      context: `${sportType}_matches`,
  ...
  // useEffect(() => {
  //   if (userData) {
  //     const favoriteSelected = userData?.data?.favorites?.[`${sportType}_matches`]?.some(
  //       (item: any) => parseInt(item.id) === parseInt(match.id)
  //     );
  //     setIsFavorite(favoriteSelected);
  //   }
  // }, [match, userData,sportType]);

  const findFavoriteFixture = (matchId: number) => {
    return !!userData?.data?.favorites?.[`${sportType}_matches`]?.find((item: any) => item.id === matchId);
  };
            ...
            <div key={match?.id}>
              <div className='grid grid-cols-5'>
                ...
                <div className='col-span-3'>
                  <Link href={`/league/${getSlugify(matchData?.data?.league?.name)}/${matchData?.data?.league?.id}`}>
                    <div className='flex items-center justify-between p-3'>
                      <div className='flex items-center gap-3 '>
                        ...
                        <div className=' '>
                          <h2 className='text-[11px] md:text-base'>{matchData?.data?.league?.name}</h2>
                          <p className='text-[9px] md:text-xs'>{matchData?.league?.country?.name}</p>
                        </div>
                      </div>
                    </div>
                  </Link>
                ...
                <div className='col-span-10 lg:col-span-3 '>
                  <div className='bg-[#1B2435]'>
                    ...
                    <div className='relative '>
                      <Link
                        className={
                          "my-2h-16 grid grid-cols-12 content-center gap-4 bg-[#1B2435] text-[11px] md:text-lg"
                        }
                        href={`/match/details/${getSlugify(match?.home_team_name)}-vs-${getSlugify(
                          ...
                          <div>
                            <div className='flex justify-between pb-2 text-gray-400'>
                              <div className='flex items-center gap-2 '>
                                ...
                                <h2>
                                  {getShortName(teamByLocation("home")?.name, teamByLocation("home")?.short_code)}
                                </h2>
                              </div>
                            </div>
                            <div className='flex justify-between text-gray-400 '>
                              <div className='flex items-center gap-2 '>
                                ...
                                <h2>
                                  {getShortName(teamByLocation("away")?.name, teamByLocation("away")?.short_code)}
                                </h2>
                              </div>
                            </div>
                          </div>
                      ...
                      {/* favorite section  */}
                      <div className={" absolute right-[0.25rem] top-[1.2rem]  h-8 w-8 lg:right-3 lg:top-6"}>
                        <div>
                          {/* <IoStar className="mx-auto my-auto text-[14px] text-yellow-500 md:text-xl" /> */}
                          {findFavoriteFixture(parseInt(match?.id)) ? (
                            <IoStar
                              onClick={(e) => handleUpdateFavorite(parseInt(match?.id))}
                              className='h-4 cursor-pointer lg:h-5 text-xl text-yellow-500'
                            />
                          ) : (
                            <IoStarOutline
                              onClick={() => handleAddToFavorite(parseInt(match?.id))}
                              className='h-4 cursor-pointer lg:h-5 text-xl  '
                            />
                          )}
                        </div>
                      </div>
