...
import { IEvent, IEventType, IFormattedEvent, INestedObject, Team } from "@/types";
...
export default function Events({ matchData, totalGoal }: { matchData: INestedObject; totalGoal: string }) {
  const awayTeam = matchData?.data?.participants?.find((team: Team | undefined) => team?.meta?.location === "away")?.id;

  const homeTeam = matchData?.data?.participants?.find((team: Team | undefined) => team?.meta?.location === "home")?.id;

  const { isLoading: matchEventsLoading, data: eventData } = useGetFixturesInfoAndCommentsQuery(matchData?.data?.id, {
  ...
  const substitution = (event: IEvent) => {
    return (
      <>
        ...
        <div className='text-[10px] font-normal md:text-base'>
          <p>{event?.player}</p>
  ...
  const yellowCard = (event: IEvent, style: string) => {
    return (
      <>
        ...
        <div>
          <h2 className='text-[10px] md:text-base'>{event?.player}</h2>
          <p className={`${style} text-[10px] md:text-sm`}>Foul</p>
        </div>
      </>
    );
  };
  ...
  const goal = (event: IEvent, style: string) => {
    return (
      <>
        ...
        <div>
          <h2 className='text-[10px] md:text-base'>{event?.player}</h2>
          <p className={`${style} text-[10px] md:text-sm`}>Goal</p>
        </div>
      </>
    );
  };
  ...
  const ownGoal = (event: IEvent) => {
    return (
      <>
        <BiFootball className='text-[10px] md:text-base' />
  ...
  const VAR_CARD = (event: IEvent) => {
    return (
      <>
        <Image src='/icons/var.png' alt='var-logo' height={0} width={0} sizes='100vw' className='w-6 rounded-md' />
        <div className='text-[10px] font-normal md:text-base'>
          <p>{event?.player}</p>
          <p>Goal Confirm</p>
        </div>
  ...
  const firstHalfAwayGoal = matchData?.data?.scores?.filter(
    (team: Team | undefined) => team?.description === "1ST_HALF" && team?.score?.participant === "away"
  );
  const firstHalfHomeGoal = matchData?.data?.scores?.filter(
    (team: Team | undefined) => team?.description === "1ST_HALF" && team?.score?.participant === "home"
  );

  const totalFirstHalfGoal = `${firstHalfHomeGoal[0]?.score?.goals ? firstHalfHomeGoal[0]?.score?.goals : 0} - ${
    firstHalfAwayGoal[0]?.score?.goals ? firstHalfAwayGoal[0]?.score?.goals : 0
  }`;

  return (
    <div className='mb-20  mt-7 md:mb-20'>
      <div className='mb-6'>
        <div className='my-2 bg-[#1B2435] py-2 text-center '>
          <h2 className=' '>Full Time</h2>
          <h2 className=' '>{totalGoal}</h2>
        </div>
        {newArray
          .sort((a, b) => {
          ...
          .map((event, index) => {
            if (Number(event.minute) > 45 + Number(matchData?.data?.periods[0]?.time_added)) {
              return (
                <div key={index} className=' mx-auto mt-3 grid h-16 w-full grid-cols-3 items-center gap-5 px-4  '>
                  <div>
                    {homeTeam === event?.participant_id ? (
                      <div
                        className={`flex ${
                          homeTeam === event?.participant_id ? "flex-row-reverse" : ""
                  ...
                  <div className='col-span-1 h-12 w-12 justify-self-center rounded-full bg-[#061A30] text-center md:h-16 md:w-16'>
                    <h4 className='mt-3 md:mt-5'>
                      {event?.minute}
                      {`'`}
                    </h4>
                  </div>
      ...
      <div>
        <div className='my-2 bg-[#1B2435] py-2 text-center'>
          <h2 className=' '>1st Half</h2>
          <h2 className=' '>{totalFirstHalfGoal}</h2>
        </div>
        {newArray
          .sort((a, b) => {
            const minuteA = a?.minute ? parseInt(a?.minute, 10) : 0;
          ...
          .map((event, index) => {
            if (Number(event.minute) < 45 + Number(matchData?.data?.periods[1]?.time_added)) {
              return (
                <div key={index} className=' mx-auto mt-3 grid h-16 w-full grid-cols-3 items-center gap-5 px-4  '>
                  <div>
                    {homeTeam === event?.participant_id ? (
