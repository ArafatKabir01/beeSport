  ...
  return (
    <div className='relative flex h-10 w-20 flex-col items-center justify-center gap-0 text-xs md:h-20 md:w-20 md:gap-1'>
      ...
      <h4 className='text-center text-[8px] font-semibold md:text-xs'>
        <span>{getShortName(player?.name)}</span>
      </h4>
      {/* ratings */}
      <div
        style={{
          backgroundColor:
            Number(player?.rating) >= 8.25
              ? "#1D8ADD"
              : Number(player?.rating) > 7.5 && Number(player?.rating) < 9
              ? "#22B96B"
              : "#F67E1D"
        }}
        className={`absolute right-[0.65rem] top-[-0.55rem] rounded-md px-1 md:right-1 md:top-1`}
      >
        <p className='my-auto text-[8px]   md:text-[10px]'>{player?.rating}</p>
      </div>
      {/* goal */}
