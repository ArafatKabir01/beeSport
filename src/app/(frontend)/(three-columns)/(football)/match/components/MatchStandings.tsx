...
export default function MatchStandings({
  ...
  const homeData = matchData?.data?.participants?.filter((data: Team) => data?.meta?.location === "home");
  const awayData = matchData?.data?.participants?.filter((data: Team) => data?.meta?.location === "away");
    ...
    <div>
      <div className=' grid grid-cols-1 gap-3 py-8  lg:grid-cols-2'>
        <div className='m-2 py-2'>
          ...
          >
            <h4 className='mx-4 font-semibold uppercase  '>{homeData[0]?.name}</h4>
            <Image
        ...
        <div className='m-2 py-2'>
          ...
          >
            <h4 className='mx-4 font-semibold uppercase  '>{awayData[0]?.name}</h4>
            <Image
      ...
      <UpcomingMatch homeMatchResponse={homeMatchResponse} awayMatchResponse={awayMatchResponse} />
    </div>
  );
}
