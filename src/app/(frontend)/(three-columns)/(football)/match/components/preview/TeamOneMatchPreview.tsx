...
export default function TeamOneMatchPreview({ matchData }: INestedObject) {
  const homeTeamId = matchData?.participants?.find(
    (team: { meta: { location: string } } | undefined) => team?.meta?.location === "home"
  )?.id;

  const { isLoading: teamInfoLoading1, data: teamInfoData1 } = useGetTeamInfoQuery(homeTeamId);
      ...
      {teamInfoLoading1 ? (
        ...
        <div className='mb-3'>
          <div className='mb-4 flex items-center justify-between p-4'>
            <h4 className='font-bold uppercase  '>{teamInfoData1?.data?.name}</h4>
              ...
              {teamResults?.map((match, index) => {
                if (match === "win") {
                  return (
                    <span
                      key={index}
                      className='flex h-4 w-4 items-center justify-center rounded-full bg-green-600 p-1 font-semibold   ring-1 ring-black md:h-7 md:w-7'
                    >
                      W
                    </span>
                  );
                  ...
                  return (
                    <span
                      key={index}
                      className='flex h-4 w-4 items-center justify-center rounded-full bg-red-600 p-1 font-semibold   ring-1 ring-black md:h-7 md:w-7'
                    >
                      L
                    </span>
                  );
          ...
          <div className='grid grid-cols-3 gap-5 px-5   lg:grid-cols-7'>
            {teamOneFinalFormat?.map((match: INestedObject) => {
              const homeGoals = match?.team_one?.goal;
              const awayGoals = match?.team_two?.goal;
              return (
                <Link
                  href={`/match/details/${getSlugify(match?.team_one?.name)}-vs-${getSlugify(
                    match?.team_two?.name
                  )}/${match?.fixtureId}`}
                  key={match?.id}
                ...
                >
                  <Image
                    ...
                    height={0}
                    sizes='100vw'
                    className='h-8 w-8 rounded-full bg-white p-1 ring-1 ring-black'
                  />
