...
export default function TeamStandingsPreview({
  ...
  const getTeamIdByLocation = (location: string): string | null => {
    ...
    const team = participants.find(
      (team: { meta: { location?: string; id?: string } } | undefined) => team?.meta?.location === location
    );
    return team && team?.id;
  };
  ...
  function transformDetailsToObj(details: INestedObject[]): Record<string, string> {
  ...
  let transformedStandings: ISingleStanding[] = leagueStandingsData?.data?.map((singleStandings: LeagueStanding) => {
  ...
  transformedStandings = transformedStandings?.filter(
    (standing) =>
      Number(standing?.teamId) === Number(participantOne) || Number(standing?.teamId) === Number(participantTwo)
  );

  return (
    <div className='mt-5  '>
      <Link href={`/league/details/${matchData?.league?.id}`} className='my-5 ml-10 flex w-fit items-center gap-4'>
        <Image
          src={matchData?.league?.image_path}
          width={0}
          height={0}
          alt='League logo'
          className='h-10 w-10 rounded-full bg-white p-1 ring-1 ring-black'
          sizes='100vw'
        />
        <div>
