...
export default function TeamTwoMatchPreview({ matchData }: INestedObject) {
  const awayTeamId = matchData?.participants?.find(
    (team: { meta: { location: string } } | undefined) => team?.meta?.location === "away"
  )?.id;

  const { isLoading: teamInfoLoading2, data: teamInfoData2 } = useGetTeamInfoQuery(awayTeamId);
      ...
      {teamInfoLoading2 ? (
        ...
        <div className='file:last:mb-3'>
          <div className='mb-4 flex items-center justify-between p-4'>
            <h4 className='font-bold uppercase  '>{teamInfoData2?.data?.name}</h4>
            <div className='flex items-center gap-2 lg:gap-5'>
              {teamResults?.map((result, index) => <MatchResultIcon key={index} result={result} />)}
            </div>
          </div>
            ...
            {teamTwoFinalFormat.map((match: INestedObject, index: number) => {
              ...
              return (
                ...
                >
                  ...
                  <span>
                    {homeGoals !== undefined && awayGoals !== undefined ? `${homeGoals} - ${awayGoals}` : "N/A"}
                  </span>
