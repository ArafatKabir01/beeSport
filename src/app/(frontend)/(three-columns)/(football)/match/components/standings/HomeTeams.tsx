...
export default function HomeTeams({ matchData }: { matchData: INestedObject }) {
  ...
  const homeData = matchData?.data?.participants?.filter((data: INestedObject) => data?.meta?.location === "home");
            ...
            <div>
              {group.standings?.length > 0 ? (
                group.standings.map((singleStandings, index) => (
                  <div
                    key={index}
                    className={`${homeData[0]?.name === singleStandings?.teamName ? "text-blue-400" : " "}`}
                  >
