...
import {
  ...
  LeagueStanding,
  ResultObject,
  TransformedStandingDetails,
  TransformedStandingsArray
} from "@/types";
import StandingsShimmer from "./StandingShimmer";

export default function AwayTeams({ matchData }: { matchData: INestedObject }) {
  const { isLoading: leagueStandingsLoading, data: leagueStandingsData } = useGetLeagueStandingQuery(
    matchData?.data.season_id,
    {
      skip: !matchData?.data.season_id
    }
  );

  const awayData = matchData?.data?.participants?.filter((data: INestedObject) => data?.meta?.location === "away");
  ...
  if (isGrouped) {
    leagueStandingsData?.data?.forEach((standings: LeagueStanding) => {
      const groupIndex = groupByGroupName.findIndex((group) => group.name === standings?.group?.name);
    ...
    <>
      {isGrouped ? (
        transformedStandings2?.slice(0, 10).map((group) => (
          <div key={group?.id} className='mb-6 ml-4 rounded-2xl  border-[1px] border-primary py-3 font-semibold'>
            <h2 className='p-2  '>{group?.groupName}</h2>
            ...
            <div>
              {group.standings?.length > 0 ? (
                group.standings.map((singleStandings, index) => (
                  <div
                    key={index}
                    className={` ${awayData[0]?.name === singleStandings?.teamName ? "text-blue-400" : " "}`}
                  >
                    <StandingTeamItem key={singleStandings?.position} singleStandings={singleStandings} index={index} />
                  </div>
                ))
              ) : (
        ...
        <div className=' rounded-2xl border-[1px] border-primary py-3'>
          ...
          <div>
            {leagueStandingsData?.data?.length > 0 ? (
              transformedStandings.slice(0, 10)?.map((singleStandings, index) => (
                <div
                  key={index}
                  className={`  ${awayData[0]?.name === singleStandings?.teamName ? "text-blue-400" : " "}`}
                >
