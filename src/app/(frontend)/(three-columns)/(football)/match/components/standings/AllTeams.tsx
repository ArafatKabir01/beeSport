...
export default function AllTeams({ matchData }: { matchData: IMatch }) {
  ...
  if (isGrouped) {
    ...
    transformedStandings = leagueStandingsData?.data.map((singleStandings: LeagueStanding) => {
    ...
    <>
      {isGrouped ? (
        transformedStandings2?.map((group) => (
          <div key={group?.id} className='mb-6 ml-4 skew-y-[1deg] font-semibold'>
            ...
            <div>
              {group?.standings?.length > 0 ? (
                group.standings.map((singleStandings, index) => (
                  <StandingTeamItem index={index} key={singleStandings?.position} singleStandings={singleStandings} />
                ))
              ) : (
