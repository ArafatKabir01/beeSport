...
export default function MatchLineup({ matchData }: { matchData: INestedObject }) {
  ...
  const { isLoading: matchLineupsLoading, data: matchLineupsData } = useGetFixturesMatchLineupByIdQuery(
    matchData?.data?.id,
    {
  ...
  const getAdjustedLineup = (apiResponse: INestedObject) => {
    const getFormation = (location: string) =>
      apiResponse.formations.find((formation: { location: string }) => formation?.location === location)?.formation;
    ...
    const addToPosition = (player: Player, team: string, position: string, type: string, details: Details[]): void => {
      ...
      const playerAssists = details?.find((topic) => topic?.type_id === 79)?.data.value;
      const playerYellowCards = details?.find((topic) => topic?.type_id === 84)?.data.value;
      const playerRedCards = details?.find((topic) => topic?.type_id === 85)?.data.value;
      const playerSubstitution = details?.find((topic) => topic?.type_id === 18)?.data.value;
      ...
      formattedLineup[team][type][position] = formattedLineup[team][type][position] || [];
    ...
    for (const team of ["home", "away"]) {
      ...
      if (formation?.split("-").length === 4) {
        ...
        formattedLineup[team].lineup.midfielder = midfielders.slice(0, splitIndex);
        formattedLineup[team].lineup.midfielder_second_half = midfielders?.slice(splitIndex);
      }
    }
  ...
  const benchedPlayers = matchLineupsData?.data?.lineups
    .filter((lineup: any) => lineup?.type?.code === "bench" && lineup?.details?.length > 0)
    .sort((a: any, b: any) => {
      const getMinutesPlayed = (player: any) =>
        player?.details?.find((stat: any) => stat?.type?.developer_name === "MINUTES_PLAYED")?.data?.value || 0;
  ...
  const homeBenchPlayers = benchedPlayers.filter((players: any) => players?.team_id === homeTeamInfo?.id);
  const awayBenchPlayers = benchedPlayers.filter((players: any) => players?.team_id === awayTeamInfo?.id);

  return (
    <div className='grid-col m-2 mb-20 mt-10 grid gap-2 text-gray-500 lg:m-2 lg:mb-0 lg:grid-cols-5'>
      <div className='col-span-5 lg:col-span-3'>
        <div className='relative mb-3  w-fit lg:mb-0'>
          <img className='h-full w-full  rounded-md' src='/images/06.png' alt='field image' />

          <div className='absolute inset-0  h-full w-full '>
            <div className='mx-auto h-full flex flex-col gap-[3%] p-4'>
              {/* Home Team */}
              <div className='relative h-full text-gray-300'>
                <div className='mx-auto flex items-center justify-center mt-[2%] gap-0 lg:gap-4'>
                  {adjustedData?.home?.lineup?.goalkeeper?.map((player: any) => (
                    <PlayerView key={player.id} player={player} />
                  ))}
                </div>
                <div className={`mx-auto flex items-center justify-center mt-[6%] gap-0 lg:gap-4`}>
                ...
                {adjustedData?.home?.lineup?.midfielder_second_half ? (
                  <>
                    <div className={`mx-auto flex items-center justify-center mb-[5%] gap-0 lg:gap-4`}>
                      {adjustedData?.home?.lineup?.midfielder_second_half.map((player: any) => (
                        <PlayerView key={player?.id} player={player} />
                      ))}
                    </div>
                    <div className={`mx-auto flex items-center justify-center mb-[5%] gap-0 lg:gap-4`}>
                      {adjustedData?.home?.lineup?.attacker.map((player: any) => (
                        <PlayerView key={player?.id} player={player} />
                      ))}
                    </div>
                  </>
                  ...
                  <>
                    <div className={`mx-auto flex items-center justify-center mt-[5%] gap-0 lg:gap-4`}>
                      {adjustedData?.home?.lineup?.attacker.map((player: any) => (
                        <PlayerView key={player?.id} player={player} />
                      ))}
                    </div>
                ...
                <div className='absolute -top-4 left-0 flex items-center gap-2 text-[11px] lg:-top-3 lg:left-5 lg:text-sm'>
                  <Image
                    width={0}
                    height={0}
                    sizes='100vw'
                    className='h-5 w-5 rounded-full p-0.5  lg:h-8 lg:w-8'
                    src={homeTeamInfo?.image}
                    alt='Logo'
                  />
                  <span className='text-gray-300'>{getShortName(homeTeamInfo?.name)}</span>
                  <span className='text-gray-300'>
                    {matchLineupsData?.data?.formations
                      ? matchLineupsData?.data?.formations
                          .filter((f: any) => f.location === "home")
                          .map((filteredFormation: any) => (
                            <div key={filteredFormation?.id}>{filteredFormation?.formation}</div>
                          ))
              ...
              <div className='h-full text-gray-300'>
                {adjustedData?.away?.lineup?.midfielder_second_half ? (
                  <>
                    <div className={`mx-auto flex items-center justify-center mb-[0%] gap-0 lg:gap-4`}>
                      {adjustedData?.away?.lineup?.attacker.map((player: any) => (
                        <PlayerView key={player?.id} player={player} />
                      ))}
                    </div>
                    <div className={`mx-auto flex items-center justify-center mb-[0%] gap-0 lg:gap-4`}>
                      {adjustedData?.away?.lineup?.midfielder_second_half.map((player: any) => (
                        <PlayerView key={player?.id} player={player} />
                      ))}
                    </div>
                  </>
                  ...
                  <>
                    <div className={`mx-auto flex items-center justify-center mb-[7%] gap-0 lg:gap-4`}>
                      {adjustedData?.away?.lineup?.attacker.map((player: any) => (
                        <PlayerView key={player?.id} player={player} />
                      ))}
                    </div>
                ...
                <div className={`mx-auto flex items-center justify-center mb-[6%] gap-0 lg:gap-4`}>
                  {adjustedData?.away?.lineup?.defender.map((player: any) => (
                    <PlayerView key={player?.id} player={player} />
                  ))}
                </div>
                <div className='mx-auto flex items-center justify-center gap-0 lg:gap-4'>
                  {adjustedData?.away?.lineup?.goalkeeper?.map((player: any) => (
                    <PlayerView key={player?.id} player={player} />
                  ))}
                </div>

                <div className='absolute right-[7%] top-[95%] flex items-center gap-2 pb-1 text-[11px] md:right-[12%] lg:top-[95.5%] lg:text-sm'>
                  <span className='text-gray-300'>
                    {matchLineupsData?.data?.formations
                      ? matchLineupsData?.data?.formations
                  ...
                  <span className='text-gray-300 '>{getShortName(awayTeamInfo?.name)}</span>
                  <Image
                    width={0}
                    height={0}
                    sizes='100vw'
      ...
      <div className='col-span-5 lg:col-span-2'>
        ...
        <div className='  rounded-2xl border-[1px] border-primary p-4'>
          ...
          <div className='my-4 grid grid-cols-2 text-[12px] lg:text-base'>
                  ...
                  <div key={playerData.id}>
                    <div className=' my-2 grid grid-cols-5 content-center items-center gap-2 '>
                      ...
                      <div className='col-span-2'>
                        <p className='me-3 text-[9px] font-medium lg:text-xs '>{playerData?.player?.display_name}</p>
                        <p className='text-[8px] lg:text-[10px]'>{playerData?.player?.position?.name}</p>
                      </div>
